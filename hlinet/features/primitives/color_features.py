...
@register_feature(name="yellow_dominant", tags=["color", "primitive"], description="Image has dominant yellow/orange/golden coloring")
class YellowDominant:
    def evaluate(self, graph: SceneGraph, region: Region | None = None) -> FeatureValue:
        yellow_coverage = 0.0
        for atom in graph.atoms:
            if atom.kind == "color_region" and atom.metadata.get("color") in ("yellow", "orange", "golden"):
        ...
        if yellow_coverage > 0.1:
            return FeatureValue.detected(
                ...
                evidence=[f"yellow/orange/golden coverage: {yellow_coverage:.2f}"],
            )
        return FeatureValue.absent("no significant yellow/orange/golden")
