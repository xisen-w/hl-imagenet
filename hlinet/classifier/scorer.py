...
def score_node(node: ClassNode, graph: SceneGraph, cache: dict[str, FeatureValue]) -> float:
    ...
    alt_penalty = 0.0
    if required_score < 0.1 and node.required_alternatives:
        for alt_features in node.required_alternatives:
            alt_score = _eval_required(alt_features, graph, cache)
            if alt_score >= 0.1:
                required_score = alt_score
                alt_penalty = 0.20
                break
...
def _eval_excluding_avg_top2(feature_names: list[str], graph: SceneGraph, cache: dict[str, FeatureValue]) -> float:
    """For excluding: average of top-2 firing features (stronger penalty from salient exclusions)."""
    if not feature_names:
        return 0.0
